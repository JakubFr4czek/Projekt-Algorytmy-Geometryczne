...
class Mbr:
    ...
    def smallest_rectangle(points, compare):
        ...
        return minRectangle, result

    def smallest_rectangle_draw(points, compare = compare_area):
        ...
        for i in range(len(minX)):
            ...
            tempRectangle = Rectangle((minX[i], minY[i]),
                                    (maxX[i], minY[i]), 
                                    (maxX[i], maxY[i]), 
                                    (minX[i], maxY[i]))

            for j in range(len(tempRectangle.vertices)):

                tempRectangle.vertices[j] = (tempRectangle.vertices[j][0] * norm_vec[i][0] + tempRectangle.vertices[j][1] * perp_vec[i][0],
                                            tempRectangle.vertices[j][0] * norm_vec[i][1] + tempRectangle.vertices[j][1] *perp_vec[i][1])

            tempEdges = tempRectangle.getEdges()

            tempFigure = vis.add_line_segment(tempEdges, color = 'black')
            vis.remove_figure(tempFigure)


        minRectangle = Rectangle((minX[minIdx],minY[minIdx]), (maxX[minIdx], minY[minIdx]), (maxX[minIdx], maxY[minIdx]), (minX[minIdx], maxY[minIdx]))
        
        for i in range(len(minRectangle.vertices)):

            minRectangle.vertices[i] = (minRectangle.vertices[i][0] * norm_vec[minIdx][0] + minRectangle.vertices[i][1] * perp_vec[minIdx][0],
                                        minRectangle.vertices[i][0] * norm_vec[minIdx][1] + minRectangle.vertices[i][1] *perp_vec[minIdx][1])

        edges = minRectangle.getEdges()
